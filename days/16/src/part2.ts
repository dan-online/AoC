...
let maxFlow = 0;
let openValves = {};
let blacklist: { [key: string]: number } = {};
const run = () => {
  while (queue.length > 0) {
    const current = queue.shift()!;
    ...
    const options = Object.values(graph).filter((valve) => valve.flow > 0 && !current.openValves[valve.name] && !blacklist[valve.name]);
...
console.log(maxFlow);
console.log(Object.keys(openValves));
console.log(valves.filter((x) => !Object.keys(openValves).includes(x.name)).map((x) => x.name));

blacklist = openValves;

const firstFlow = maxFlow;
